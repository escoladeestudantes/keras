#Usar
#python retinanet_edit_video.py
...
#Remover ''' abaixo para baixar e extrair o arquivo
'''
...
with zipfile.ZipFile("data.zip", "r") as z_fp:
    z_fp.extractall("./")
'''
#Remover ''' acima para baixar e extrair o arquivo
...
def visualize_detections_video(
    image, boxes, classes, scores, figsize, linewidth, dic_classes
):
    ...
    for box, _cls, score in zip(boxes, classes, scores):
        ...
        if (_cls == 'motorcycle'):
            _cls = 'motorbike'
        color = [int(c) for c in dic_classes.get(_cls)]
...
coco_labels = open('coco.names').read().strip().split('\n')
colors = np.random.randint(0, 255, size=(len(coco_labels), 3), dtype='uint8')
dicclasses = {}
for idx, lbl in enumerate(coco_labels):
    dicclasses[lbl] = tuple(colors[idx])
...
while (ret==True):
    if (stop == 0):
        ...
        int2str(int(x)) for x in detections.nmsed_classes[0][:num_detections]]
            visualize_detections_video(image,detections.nmsed_boxes[0][:num_detections] / ratio,class_names,detections.nmsed_scores[0][:num_detections], (7, 7), 1, dicclasses)
